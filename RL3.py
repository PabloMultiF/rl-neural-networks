...
class Agent:
    ...
    def update_q_table(self, old_state, new_state, reward, action):
        index_action = list(self.environment.actions).index(action)

        actual_state_q_table_value = self.q_table[old_state[0]][old_state[1]][index_action]

        future_state_q_table_value = max(self.q_table[new_state[0]][new_state[1]])
    ...
    def run(self, n_episodes = 10):
        ...
        for _ in range(0, n_episodes):
            ...
            while not final_state:
                ...
                self.update_q_table(old_state, new_state, reward, action)
